...
Ste::Ste(const Property & p, const TransitionSystem & ts,
...
void Ste::initialize()
{
  ...
  logger.log(2, "BMC adding init constraint for step 0 {}", unroller_.at_time(ts_.init(), 0)->to_string());
  solver_->assert_formula(unroller_.at_time(ts_.init(), 0));
}

ProverResult Ste::check_until(int k)
{
  initialize();
  ...
  logger.log(1, "STE bound_start_: {} ", bound_start_);
  logger.log(1, "STE bound_step_: {} ", bound_step_);
...
Term con(int i ,const Term & x, const SmtSolver & solver){
  return solver->make_term(Equal,x,solver->make_term(i,x->get_sort()));
}

bool Ste::step(int i)
{
  logger.log(1, "\nSTE checking at bound: {}", i);
  ...
  if (i > 0) {
    ...
    logger.log(2, "  STE reached_k = {}, i = {} ", reached_k_, i);
    for (int j = reached_k_ == -1 ? 1 : reached_k_ + 1; j <= i; j++) {
      logger.log(2, "  STE adding transition for j-1 = {}", j - 1);
//      logger.log(3," j:  {}",j);
      auto czh = unroller_.at_time(ts_.trans(), j - 1);
//      logger.log(3,"        trans:"+czh->to_string());
      solver_->assert_formula(czh);
//      logger.log(3,"        trans:"+czh->to_string());
//      logger.log(3,"        trans:"+ts_.trans()->to_string());
//      logger.log(3," j:  {} done",j);
   ...
   * czh
   */
  Term ant_i = unroller_.at_time(antecedent[i],i);
  //  Term con_i = unroller_.at_time(con(i,x,solver_),i);
  logger.log(3,"     push ant:"+ant_i->to_string());
  solver_->assert_formula(ant_i);
  solver_->push();

  Result ant_r = solver_->check_sat();
  if (!ant_r.is_sat()) {
    logger.log(1, "  antecedent check at bound {} unsatisfiable, 存在前提冲突", i);
    return false;
  }
  ...
  /**
     * czh
   */
  Term con_i = unroller_.at_time(consequent[i],i);
  logger.log(3,"     push cons :"+con_i->to_string());
  //  Term con_i = unroller_.at_time(con(i,x,solver_),i);
//  logger.log(3,"     push cons:"+con_i->to_string());
  Term not_con_i = solver_->make_term(Not,con_i);
  solver_->assert_formula(not_con_i);
...
//      std::cout<<unroller_.at_time(ts_.trans(), j - 1)->to_string()<<std::endl;
//    }
//  }
//
//  /**
//     * czh
//   */
//  Term x = ts_.named_terms().at("internal");
//  Term con_i = unroller_.at_time(con(i,x,solver_),i);
//  Term not_con_i = solver_->make_term(Not,con_i);
//  auto z = not_con_i->to_string();
//  std::cout<<"cons : "<<not_con_i->to_string()<<std::endl;
//  solver_->assert_formula(not_con_i);
//
//
//
...
//
//  //solver_->assert_formula(clause);
//  std::cout<<"p: "<<clause->to_string()<<std::endl;
//
//  Result r = solver_->check_sat();
//
//  if (r.is_sat()) {
...
int Ste::bmc_interval_get_cex_ub(const int lb, const int ub)
{
...
void Ste::bmc_interval_block_cex_ub(const int start, const int end)
{
...
bool Ste::find_shortest_cex_binary_search(const int upper_bound)
{
...
bool Ste::find_shortest_cex_binary_search_less_inc(const int upper_bound)
{
  assert (reached_k_ < upper_bound);
...
void Ste::find_shortest_cex_linear_search(const int upper_bound)
{  
  assert (reached_k_ < upper_bound);
