...
#include "utils/formula_compiler.cpp"
...
void ste_mult()
{
  ...
  //function
  auto AND = [s](Term a, Term b){
    return s->make_term(And,a,b);
  };
  auto ANDV = [s](vector<Term> v){
    return s->make_term(And,v);
  };
  auto OR = [s](Term a, Term b){
    return s->make_term(Or, a,b);
  };
  auto NOT = [s](Term a){
    return s->make_term(Not,a);
  };
  auto IMPLY = [s](Term a, Term b){
    return s->make_term(Implies,a,b);
  };
  auto EQUAL = [s](Term a, Term b){
    return s->make_term(Equal,a,b);
  };
  ...
  cout<<s->make_term(BVMul,ZEROEXTEND(a,4),ZEROEXTEND(b,4))->to_string()<<endl;
  for(int i=0;i<n;i++){
    if(i==5){
      ste.consequent.push_back(EQUAL(io_res,s->make_term(BVMul,ZEROEXTEND(a,4),ZEROEXTEND(b,4))));
    }else{
